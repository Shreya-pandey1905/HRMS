...
<body>
													...
													<ul>
														<li class="submenu">
															...
															<ul>
																...
																<li><a href="leaves-employee.html" class="active">Leave (Employee)</a>
																</li>
    ...
    <ul>
						...
						<li class="submenu">
							...
							<ul>
								...
								<li class="submenu">
									...
									<ul>
										<li class="submenu">
											<a href="javascript:void(0);" class="active subdrop">Leaves<span class="menu-arrow"></span></a>
											<ul>
												<li><a href="leaves.html">Leaves (Admin)</a></li>
												<li><a href="leaves-employee.html" class="active">Leave (Employee)</a></li>
							...
							<div class="tab-content">
								...
								<div class="tab-pane fade show active" id="menu-hrm">
									<ul class="stack-submenu">
										...
										<li><a href="holidays.html"><span>Holidays</span></a></li>
										<li class="submenu">
											<a href="javascript:void(0);" class="active"><span>Attendance</span>
												<span class="menu-arrow"></span>
											</a>
											<ul>
												<li class="submenu submenu-two">
													<a href="javascript:void(0);" class="active">Leaves<span
															class="menu-arrow inside-submenu"></span></a>
													<ul>
														<li><a href="leaves.html">Leaves (Admin)</a></li>
														<li><a href="leaves-employee.html" class="active">Leave (Employee)</a></li>
				...
				<div class="row">
					<c:choose>
						<c:when test="${not empty leaveBalances}">
							<c:forEach var="balance" items="${leaveBalances}" varStatus="loop">
								<div class="col-xl-3 col-md-6">
									<div class="card ${loop.index % 4 == 0 ? 'bg-black-le' : loop.index % 4 == 1 ? 'bg-blue-le' : loop.index % 4 == 2 ? 'bg-purple-le' : 'bg-pink-le'}">
										<div class="card-body">
											<div class="d-flex align-items-center justify-content-between">
												<div class="text-start">
													<p class="mb-1">${balance.leaveType}</p>
													<h4>${balance.usedLeaves}</h4>
													<span class="fs-12">Taken / ${balance.totalLeaves}</span>
												</div>
												<span class="avatar avatar-md d-flex"><i class="ti ti-calendar-event fs-32"></i></span>
											</div>
											<span class="badge bg-secondary-transparent">Remaining Leaves : ${balance.remainingLeaves}</span>
										</div>
									</div>
								</div>
							</c:forEach>
						...
						<c:otherwise>
							<div class="col-12"><div class="alert alert-info">No leave has been allocated to your department yet.</div></div>
						</c:otherwise>
					</c:choose>
				</div>
				<!-- /Leaves Info -->
				...
				<div class="card">
					<div class="card-header">
						<div class="d-flex align-items-center justify-content-between flex-wrap row-gap-3 w-100">
							<div class="d-flex align-items-center">
								<h5 class="me-2 mb-0">Leave List</h5>
								<span class="badge bg-primary-transparent me-2">Total Allocated : ${totalLeaves}</span>
								<span class="badge bg-secondary-transparent">Total Remaining : ${totalRemainingLeaves}</span>
							</div>
						</div>
						<form method="get" action="${pageContext.request.contextPath}/employee/leave" class="row g-2 mt-2 align-items-end">
							<div class="col-md-2">
								<label class="form-label mb-1">From</label>
								<input type="date" name="fromDate" value="${selectedFromDate}" class="form-control form-control-sm">
							</div>
							<div class="col-md-2">
								<label class="form-label mb-1">To</label>
								<input type="date" name="toDate" value="${selectedToDate}" class="form-control form-control-sm">
							</div>
							<div class="col-md-2">
								...
								<select name="leaveTypeId" class="form-select form-select-sm">
									...
									<c:forEach var="type" items="${leaveTypes}">
										<option value="${type.leaveTypeId}" ${selectedLeaveTypeId == type.leaveTypeId ? 'selected' : ''}>${type.leaveType}</option>
									</c:forEach>
								</select>
							</div>
							<div class="col-md-2">
							...
							<div class="col-md-2 d-flex gap-2">
								...
								<a href="${pageContext.request.contextPath}/employee/leave" class="btn btn-sm btn-light">Reset</a>
							</div>
						</form>
					</div>
						...
						<div class="custom-datatable-filter table-responsive">
							<table class="table datatable">
								<thead class="thead-light">
									<tr><th>Leave Type</th><th>From</th><th>Approved By</th><th>To</th><th>No of Days</th><th>Status</th><th>Reason</th></tr>
								</thead>
								<tbody>
									<c:forEach var="record" items="${leaveRecords}">
										<tr>
											<td class="fs-14 fw-medium">${record.leaveType}</td>
											<td>${record.startDateLabel}</td>
											<td>${empty record.approvedBy ? 'Pending' : record.approvedBy}</td>
											<td>${record.endDateLabel}</td>
											<td>${record.numberOfDays} Days</td>
											<td><span class="badge ${record.status == 'Approved' ? 'badge-soft-success' : record.status == 'Declined' ? 'badge-soft-danger' : 'badge-soft-warning'}">${record.status}</span></td>
											<td><span class="text-truncate d-inline-block" style="max-width:260px" title="${record.reason}">${record.reason}</span></td>
										</tr>
									</c:forEach>
									<c:if test="empty leaveRecords"><tr><td colspan="7" class="text-center py-4">No leave records found for the selected filters.</td></tr></c:if>
								</tbody>
							</table>
						</div>
					</div>
		...
		<!-- Add Leaves -->
		<div class="modal fade" id="add_leaves" tabindex="-1">
			<div class="modal-dialog modal-dialog-centered modal-lg">
				<div class="modal-content">
					<div class="modal-header"><h4 class="modal-title">Add Leave</h4><button type="button" class="btn-close custom-btn-close" data-bs-dismiss="modal" aria-label="Close"><i class="ti ti-x"></i></button></div>
					<form method="post" action="${pageContext.request.contextPath}/employee/leave" id="addLeaveForm">
						<div class="modal-body pb-0"><div class="row">
							<div class="col-md-12"><div class="mb-3"><label class="form-label">Leave Type</label><select name="leaveTypeId" id="leaveTypeId" class="form-select" required><option value="">Select</option><c:forEach var="type" items="${leaveTypes}"><option value="${type.leaveTypeId}">${type.leaveType}</option></c:forEach></select></div></div>
							<div class="col-md-6"><div class="mb-3"><label class="form-label">From</label><input type="date" name="fromDate" id="leaveFromDate" class="form-control" required></div></div>
							<div class="col-md-6"><div class="mb-3"><label class="form-label">To</label><input type="date" name="toDate" id="leaveToDate" class="form-control" required></div></div>
							<div class="col-md-12"><div class="mb-3"><div id="leaveDayPreview" class="text-muted small">Select dates to see payable leave days. Sundays are excluded automatically.</div></div></div>
							<div class="col-md-12"><div class="mb-3"><label class="form-label">Reason</label><textarea name="reason" class="form-control" rows="3" maxlength="2000" required></textarea></div></div>
						</div></div>
						<div class="modal-footer"><button type="button" class="btn btn-light me-2" data-bs-dismiss="modal">Cancel</button><button type="submit" class="btn btn-primary">Add Leave</button></div>
					</form>
				</div>
			</div>
		</div>
...
(function () {
    ...
    var preview = document.getElementById('leaveDayPreview');
    function countWeekdays() {
        if (!from || !to || !preview || !from.value || !to.value) return;
        var start = new Date(from.value + 'T00:00:00');
        var end = new Date(to.value + 'T00:00:00');
        if (start > end) { preview.textContent = 'End date cannot be before start date.'; return; }
        var count = 0, d = new Date(start);
        while (d <= end) { if (d.getDay() !== 0) count++; d.setDate(d.getDate() + 1); }
        preview.textContent = count === 0 ? 'Only Sunday holidays are selected; this request will be rejected.' : count + ' leave day(s) will be counted. Sundays are excluded.';
    }
    if (from) from.addEventListener('change', countWeekdays);
    if (to) to.addEventListener('change', countWeekdays);
})();
</script>
